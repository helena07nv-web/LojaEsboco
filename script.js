...
function fazerCadastro() {
  ...
  if (senha.length < 6) {
    showToast('⚠️ Senha muito curta! Mínimo 6 caracteres.');
    return;
  }
...
function addCart(btn) {
  ...
  var existente = cartItems.find(function(i) {
    return i.nome === nome;
  });
...
function atualizarBadge() {
  ...
  // HOME
  var badge1 = document.getElementById('cart-count');
      ...
      div.innerHTML = `
        <div class="cart-item-produto">
          ...
          <div class="cart-item-info">
            ...
            <button class="cart-item-remover" onclick="removerItem(${index})">
              🗑️ Remover
            </button>
          </div>
        </div>
        ...
        <div class="cart-item-qty">
          ...
          <button class="qty-btn" onclick="alterarQty(${index}, -1)">
            −
          </button>
...
function atualizarResumo() {
  ...
  var subtotal = cartItems.reduce(function(sum, i) {
    return sum + i.preco * i.qty;
  }, 0);

  var totalQtd = cartItems.reduce(function(sum, i) {
    return sum + i.qty;
  }, 0);
  ...
  document.getElementById('summary-qtd-label').textContent =
    'Subtotal (' + totalQtd + ' produto' + (totalQtd !== 1 ? 's' : '') + ')';

  document.getElementById('summary-subtotal').textContent =
    'R$ ' + subtotal.toFixed(2).replace('.', ',');

  document.getElementById('summary-desconto').textContent =
    '– R$ ' + descontoAtual.toFixed(2).replace('.', ',');

  document.getElementById('summary-frete').textContent =
    frete === 0
      ? 'Grátis'
  ...
  document.getElementById('summary-parcelamento').textContent =
    total > 0
...
function aplicarCupom() {
  ...
  var codigo = document.getElementById('coupon-input')
    .value
    .trim()
    .toUpperCase();
  ...
  if (cupons[codigo] !== undefined) {
    ...
    showToast(
      '✅ Cupom ' + codigo +
      ' aplicado! Desconto de R$ ' + descontoAtual
    );
